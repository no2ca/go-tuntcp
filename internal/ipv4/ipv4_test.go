...
func TestParseIPv4Header(t *testing.T) {
	...
	pkt := append(append([]byte{}, validHeader...), payload...)
...
func TestParseIPv4Header_Errors(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if _, _, err := Parse(tt.pkt); err == nil {
...
func TestVerifyIPv4Checksum(t *testing.T) {
	if !VerifyChecksum(validHeader) {
		t.Error("valid header reported as invalid")
	}
	bad := append([]byte{}, validHeader...)
	bad[19] ^= 0x01
...
func TestHeader_Serialize(t *testing.T) {
	payload := bytes.Repeat([]byte{0xab}, 20)
	want := append(append([]byte{}, validHeader...), payload...)
	...
	if !VerifyChecksum(got[:20]) {
		t.Error("serialized header has invalid checksum")
	}
}

func TestHeader_Serialize_IgnoresInputChecksum(t *testing.T) {
	h := validStruct
	h.Checksum = 0xdead
	got := h.Serialize(nil)
...
func TestHeader_RoundTrip(t *testing.T) {
	...
	parsed, gotPayload, err := Parse(buf)
	if err != nil {
		t.Fatalf("Parse: %v", err)
	}
...
func TestHeader_StringProtocol(t *testing.T) {
	tests := []struct {
		proto uint8
		want  string
	}{
